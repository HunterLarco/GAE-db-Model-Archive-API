...
def stats():
	...
	def nextBackup():
		difftime = datetime.datetime(
			year=((datetime.datetime.now()-datetime.timedelta(hours=5))+datetime.timedelta(days=1)).year,
			month=((datetime.datetime.now()-datetime.timedelta(hours=5))+datetime.timedelta(days=1)).month,
			day=((datetime.datetime.now()-datetime.timedelta(hours=5))+datetime.timedelta(days=1)).day,
			hour=0,
			minute=0,
			second=0
		)-datetime.datetime.now()+datetime.timedelta(hours=5)
		return str(int(floor(float(difftime.seconds)/60/60)))+':'+str(int(round(60*(float(difftime.seconds)/60/60-int(floor(float(difftime.seconds)/60/60))),0)))
	total = blobstore.BlobInfo.all()
	return dict(
		num=total.count(),
		last=((total.order('-creation').fetch(1)[0].creation-datetime.timedelta(hours=5)).strftime("%B %d, %Y, %I:%M %p")+' US/Eastern') if total.order('-creation').count() > 0  else 'No Previous Backups',
		next=nextBackup(),
		size=totalSize()
	)
def requestToken():
...
def fetch(token):
	...
	def Blob(i):
		...
		blob.stats = dict(
			size=size(),
			creation=(blob.creation-datetime.timedelta(hours=5)).strftime("%B %d, %Y, %I:%M %p"),
